...
Next, choose which database you prefer to use. The options include `MongoDB`, `MySQL`, and `PostgreSQL`. In this example, we will choose `PostgreSQL`.

![Database selection screenshot](../.github/images/database.png)

### Step 5: Select an ORM

Choose an Object-Relational Mapping (ORM) tool that you'd like to use with your database. You can select from `Mongoose`, `Prisma`, or `TypeORM`. For this example, although we've chosen PostgreSQL as our database, we'll select `Mongoose` to demonstrate the process.
